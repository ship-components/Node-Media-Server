...
class NodeRelayServer {
  ...
  run() {
    try {
      ...
      Logger.error(
        `Node Media Relay Server startup failed. ffmpeg:${
          this.config.relay.ffmpeg
        } cannot be executed.`
      );
      return;
    }
  ...
  onStatic() {
    ...
    while (i--) {
      ...
      if (isStatic) {
        ...
        conf.ouPath = `rtmp://127.0.0.1:${this.config.rtmp.port}/${conf.app}/${
          conf.name
        }`;
