...
class NodeRtmpSession {
      ...
      if (!this.isIPC) {
        Logger.log(
          `[rtmp publish] Handle audio. id=${this.id} streamPath=${
            this.publishStreamPath
          } sound_format=${sound_format} sound_type=${sound_type} sound_size=${sound_size} sound_rate=${sound_rate} codec_name=${
            this.audioCodecName
          } ${this.audioSamplerate} ${this.audioChannels}ch`
        );
      }
    }
  ...
  rtmpVideoHandler() {
    ...
    if (!this.isFirstVideoReceived) {
      ...
      if (!this.isIPC) {
        Logger.log(
          `[rtmp publish] Handle video. id=${this.id} streamPath=${
            this.publishStreamPath
          } frame_type=${frame_type} codec_id=${codec_id} codec_name=${
            this.videoCodecName
  ...
  onPublish(invokeMessage) {
    ...
    const asyncHandler = err => {
      if (err) {
        throw err;
      }
      if (
        this.config.auth &&
        this.config.auth.publish &&
        !this.isLocal &&
        !this.isIPC
      ) {
        ...
        if (!results) {
          Logger.log(
            `[rtmp publish] Unauthorized. id=${this.id} streamPath=${
              this.publishStreamPath
            } streamId=${this.publishStreamId} sign=${this.publishArgs.sign} `
          );
          this.sendStatusMessage(
      ...
      if (context.publishers.has(this.publishStreamPath)) {
        Logger.log(
          `[rtmp publish] Already has a stream. id=${this.id} streamPath=${
            this.publishStreamPath
        ...
        Logger.log(
          `[rtmp publish] NetConnection is publishing. id=${
            this.id
          } streamPath=${this.publishStreamPath} streamId=${
            this.publishStreamId
          }`
        );
        this.sendStatusMessage(
        ...
        if (!this.isIPC) {
          Logger.log(
            `[rtmp publish] New stream. id=${this.id} streamPath=${
              this.publishStreamPath
            } streamId=${this.publishStreamId}`
          );
        }
  ...
  onPlay(invokeMessage) {
    ...
    if (
      ...
      if (!results) {
        Logger.log(
          `[rtmp play] Unauthorized. id=${this.id} streamPath=${
            this.playStreamPath
          }  streamId=${this.playStreamId} sign=${this.playArgs.sign}`
        );
        this.sendStatusMessage(
          this.playStreamId,
          'error',
    ...
    if (this.isPlaying) {
      if (!this.isIPC) {
        Logger.log(
          `[rtmp play] NetConnection is playing. id=${this.id} streamPath=${
            this.playStreamPath
          }  streamId=${this.playStreamId} `
        );
      }
      this.sendStatusMessage(
        this.playStreamId,
    ...
    if (context.publishers.has(this.playStreamPath)) {
      ...
      Logger.log(
        `[rtmp play] Stream not found. id=${this.id} streamPath=${
          this.playStreamPath
        }  streamId=${this.playStreamId}`
      );
      this.isIdling = true;
  ...
  onStartPlay() {
    ...
    if (!this.isIPC) {
      ...
      Logger.log(
        `[rtmp play] Join stream. id=${this.id} streamPath=${
          this.playStreamPath
        }  streamId=${this.playStreamId} `
      );
    }
  ...
  onPause(invokeMessage) {
    ...
    Logger.log(
      `[rtmp play] ${d} stream. id=${this.id} streamPath=${
        this.playStreamPath
      }  streamId=${this.playStreamId} `
    );
    if (!this.isPause) {
  ...
  onDeleteStream(invokeMessage) {
    if (invokeMessage.streamId == this.playStreamId) {
      ...
      if (!this.isIPC) {
        Logger.log(
          `[rtmp play] Close stream. id=${this.id} streamPath=${
            this.playStreamPath
          } streamId=${this.playStreamId}`
        );
      }
    ...
    if (invokeMessage.streamId == this.publishStreamId) {
      if (this.isPublishing) {
        if (!this.isIPC) {
          Logger.log(
            `[rtmp publish] Close stream. id=${this.id} streamPath=${
              this.publishStreamPath
            } streamId=${this.publishStreamId}`
          );
          context.nodeEvent.emit(
